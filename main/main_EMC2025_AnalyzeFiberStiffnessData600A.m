...
for i=1:1:length(config)
    ...
    fiberProperties.lceMM   = ...
        trialData600A.Setup_Parameters.Fiber_Length.Value;

    fiberProperties.radiusMM= ...
        trialData600A.Setup_Parameters.Diameter.Value * 0.5;

    fiberProperties.areaMM2= pi*pow2(fiberProperties.radiusMM,2);
    ...
    for j=1:1:length(config(i).plots)
        ...
        if(config(i).plots(j).impedance.analyze==1)
            ...
            [gainNorm,phaseNorm,coherenceSqNorm] = evaluateGainPhaseCoherenceSq(...
                                        xNormTimeDomain,...
                                        yNormTimeDomain,...
                                        trialBandwidth(1,2),...
                                        sampleFrequency);
            ...
            %%
            activeFiberModulus=calcEMC2025ActiveFiberModulus600A(...
                                   xTimeDomain,...
                                   yTimeDomain,...                                   
                                   ...
                                   yTimeDomainMean(1,1),...
                                   fiberProperties);
            assert(strcmp(activeFiberModulus.Unit,'Pa'),...
                'Error: Active fiber modulus should be in Pa');


            if(config(i).plots(j).impedance.isActive==1)
                expAFK.x =[expAFK.x;xNormTimeDomainMean];
                expAFK.y =[expAFK.y;activeFiberModulus.Value];
                expFL.x =[expFL.x; xNormTimeDomainMean];
                expFL.y =[expFL.y; yNormTimeDomainMean];                
            else
                expFpeK.x =[expFpeK.x;xNormTimeDomainMean];
                expFpeK.y =[expFpeK.y;activeFiberModulus.Value];
                expFpe.x =[expFpe.x; xNormTimeDomainMean];
                expFpe.y =[expFpe.y; yNormTimeDomainMean];                
            end
            ...
            if(mean(coherenceSq(idxFreqBand,1)) > 0.9...
                ...
                %
                rho = 1062; %kg /m^3
                waveVelocityMPS = sqrt(activeFiberModulus.Value/rho);                
