...
for i=1:1:length(config)
    ...
    for j=1:1:length(config(i).plots)
        ...
        if(config(i).plots(j).impedance.analyze==1)
            ...
            %[freqHzNorm,gainNorm,phaseNorm,coherenceSqNorm] = ...
            HsNorm =  evaluateGainPhaseCoherenceSq( xNormTimeDomain,...
                                                    yNormTimeDomain,...
                                                    dataConfig.bandwidthHz(i,2),...
                                                    sampleFrequency);
            
            %[freqHz,gain,phase,coherenceSq] = ...
            Hs = evaluateGainPhaseCoherenceSq(  xTimeDomain,...
                                                yTimeDomain,...
                                                dataConfig.bandwidthHz(i,2),...
                                                sampleFrequency);
            ...
            idxFreq     = find(freqHz < (dataConfig.bandwidthHz(i,2)+dfreq));
            idxFreqBand = find(freqHz < dataConfig.bandwidthHz(i,2) ...
                             & freqHz > dataConfig.bandwidthHz(i,1));
            ...
            %[freqHzNormShift, gainNormShift,phaseNormShift,coherenceSqNormShift]...
            HsShift = evaluateGainPhaseCoherenceSq( xNormTimeDomain,...
                                                    yNormTimeDomainShift,...
                                                    dataConfig.bandwidthHz(i,2),...
                                                    sampleFrequency);
            ...
            plot(HsNorm.frequencyHz(idxFreq,:),...
                 HsNorm.gain(idxFreq,:),...
                 '-','Color',gainColor,...
                 'LineWidth',0.5);
            hold on;
            if(isempty(settingTimeShift)==0)
                plot(HsShift.frequencyHz(idxFreq,:),...
                     HsShift.gain(idxFreq,:),...
                     '-.','Color',gainColor,...
                     'LineWidth',0.5);
                hold on;
            end
            ...
            plot(HsNorm.frequencyHz(idxFreq,:),...
                 HsNorm.phase(idxFreq,:).*(180/pi),...
                 '-','Color',phaseColor,...
                 'LineWidth',0.5);
            hold on;
            if(isempty(settingTimeShift)==0)
                plot(HsShift.frequencyHz(idxFreq,:),...
                     HsShift.phase(idxFreq,:).*(180/pi),...
                     '.-','Color',phaseColor,...
                     'LineWidth',0.5);
            end
            plot([1,1].*dataConfig.bandwidthHz(i,1),...
                 max(HsShift.phase(idxFreq,:).*(180/pi)),...
                 '--k');
            hold on; 
            plot([1,1].*dataConfig.bandwidthHz(i,2),...
                 max(HsShift.phase(idxFreq,:).*(180/pi)),...
