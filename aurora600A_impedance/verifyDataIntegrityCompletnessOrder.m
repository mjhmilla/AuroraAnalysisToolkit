...
setOfVerifiedTrials = [];
trialOrdering(length(experimentJson.measurements)) = struct('name','','time','');
for i=1:1:length(experimentJson.measurements)
    ...
    if(flag_checkFileOrder==1)
      
      auroraData = readAuroraData600A(dataPath,1); 
