...
if(verbose==1)
  fprintf('%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n',...
          'No. ','Meas.','Seq. ','f(meas.name)','f(md.name)',...
          'f(seq)','f(tag)','f(all)','Name');
  
  fprintf(fidScanLog,'%s,%s,%s,%s,%s,%s,%s,%s,%s\n',...
          'No. ','Meas.','Seq. ','f(meas name)','f(md name)','f(seq)',...
          'f(tag)','f(all)','Name');
...
scanResult = struct('indexTrial',zeros(100,1),...
                    ...
                    'doesMeasurementNamePassFilter',zeros(100,1),...
...
while(isLastMeasurement==0)
  ...
  isMetaDataFileValid = applyKeywordFilter(metaDataFileName,...
                        keyWordFilter.metaDataFileName);
  
  isMeasurementNameValid= ...
    applyKeywordFilter(metaDataCache.measurementFileName,...
                       keyWordFilter.measurement);
  ...
  isPassingAllFilters = ...
    (isMeasurementNameValid == 1 && isMetaDataFileValid == 1 ...
     && validSegmentCount > 0 && validTagCount > 0);
  ...
  if(verbose==1)    
    ...
    fprintf(fidScanLog,'%i,%i,%i,%i,%i,%i,%i,%i,%s\n',...
      idxTrial,...
      metaDataCache.indexMeasurement, ...
      metaDataCache.indexSequence, ...
      isMeasurementNameValid,...
  ...
  if(idxTrial > length(scanResult.indexTrial))    
    ...
    scanResult.doesMeasurementNamePassFilter= ...
      [scanResult.doesMeasurementNamePassFilter;zeros(100,1)];    
  ...
  scanResult.doesMeasurementNamePassFilter(idxTrial,1)=isMeasurementNameValid;
  scanResult.doesFileNamePassFilter(idxTrial,1)= isMetaDataFileValid;
...
scanResult.doesMeasurementNamePassFilter = ...
  scanResult.doesMeasurementNamePassFilter(1:idxTrial);
scanResult.doesFileNamePassFilter= ...
