    ...
    if(scanSummary.doesFileNamePassFilter(metaDataCache.indexTrial)==1)
      ...
      if(targetTrialCount>0)
        ...
        if(~isempty(dataInfo.L.settings))
          lengthUnit = dataInfo.L.settings.unit;
          lengthScale = dataInfo.L.settings.scale_units_per_V;
        else
        ...
        if(~isempty(dataInfo.F.settings))
          forceUnit = dataInfo.F.settings.unit;
          forceScale = dataInfo.F.settings.scale_units_per_V;
        else
        ...
        yyaxis left;
          ...
          if(yLeftDataLimits(1,1)>min(ddfData610.data.(dataInfo.L.ch).Values))
            yLeftDataLimits(1,1)=min(ddfData610.data.(dataInfo.L.ch).Values);
          end
          if(yLeftDataLimits(1,2)<max(ddfData610.data.(dataInfo.L.ch).Values))
            yLeftDataLimits(1,2)=max(ddfData610.data.(dataInfo.L.ch).Values);
          end
        ...
        yyaxis right;  
          ...
          if(sum(ddfData610.data.(dataInfo.SS.ch).Values)>0)  
            plot(timeSeries, ...
                ddfData610.data.(dataInfo.SS.ch).Values.*settings.stimulusDataScale,...
                 '-','Color',[1,1,1].*0.25);
            hold on;
          end          
          ...
          if(~isempty(dataInfo.SV.ch))
            plot(timeSeries, ...
                ddfData610.data.(dataInfo.SV.ch).Values...
                .*settings.stimulusVoltageDataScale,...
                 '-','Color',colors.SV);                     
            hold on;
            [maxV,idxMaxV]=max(abs(ddfData610.data.(dataInfo.SV.ch).Values));
  
            idxMid = round(length(timeSeries)*0.5);
            text(timeSeries(end),...
                 maxV.*settings.stimulusVoltageDataScale,...
                 sprintf('%s%1.1f%s','$$\pm$$',maxV*dataInfo.SV.settings.scale_units_per_V,...
                                   dataInfo.SV.settings.unit),...
                 'FontSize',6,...
          ...
          if(~isempty(dataInfo.SC.ch))
            plot(timeSeries, ...
                ddfData610.data.(dataInfo.SC.ch).Values...
            ...
            [maxA,idxMaxA]=max(abs(ddfData610.data.(dataInfo.SC.ch).Values));
            text(timeSeries(end),...
                 -maxA.*settings.stimulusCurrentDataScale,...
                 sprintf('%s%1.1f%s','$$\pm$$',maxA*dataInfo.SC.settings.scale_units_per_V,...
                                   dataInfo.SC.settings.unit),...
                 'FontSize',6,...
                 'HorizontalAlignment','right',...
                 'VerticalAlignment','bottom',...
                 'Color',colors.SC);
          ...
          if(settings.annotateMinMaxTrialForce==1)
            ...
            plot(timeSeries(idxMax),...
              ddfData610.data.(dataInfo.F.ch).Values(idxMax),...
            ...
            [minF, idxMin] = min(ddfData610.data.(dataInfo.F.ch).Values);
            plot(timeSeries(idxMin),...
              ddfData610.data.(dataInfo.F.ch).Values(idxMin),...
              'xk','MarkerFaceColor',[0,0,0]);
            hold on;
          ...
          chLabel = {'F'};          
          if(~isempty(dataInfo.F.ch) ...
              && ~isempty(dataInfo.SV.ch) ...
              && ~isempty(dataInfo.SA.ch))
            chLabel = {'F','SV','SC'};
          end
          for k=1:1:length(chLabel)
            minYValues(k)=min(ddfData610.data.(dataInfo.(chLabel{k}).ch).Values);
            maxYValues(k)=max(ddfData610.data.(dataInfo.(chLabel{k}).ch).Values);
          ...
          if(isSegmentValid)
            ...
            yyaxis left;
              ...
              plot(timeSeries(idxTime), ...
                ddfData610.data.(dataInfo.L.ch).Values(idxTime),...
                '-','Color',colors.L);  
              hold on;
              ylabel(['Length (',lengthUnit,')']);
              ...
              yyaxis right;
                ...
                if(~isempty(dataInfo.SC.ch))
                  ...
                  hold on;
                  idxMid = round(length(timeSeries(idxTime))*0.5);
                  [maxA,idxMaxA]=max(ddfData610.data.(dataInfo.SC.ch).Values(idxTime));
                  text(timeSeries(idxTime(end)),...
                       -maxA.*settings.stimulusCurrentDataScale,...
                ...
                if(settings.annotateMinMaxSegmentForce==1)
                  ...
                  plot(timeSeries(idxTime(idxMax)),...
                    ddfData610.data.(dataInfo.F.ch).Values(idxTime(idxMax)),...
                    'xk','MarkerFaceColor',[0,0,0]);
                  hold on;
                  ...
                  [minF, idxMin] = min(ddfData610.data.(dataInfo.F.ch).Values(idxTime));
                  plot(timeSeries(idxTime(idxMin)),...
                    ddfData610.data.(dataInfo.F.ch).Values(idxTime(idxMin)),...
                    'xk','MarkerFaceColor',[0,0,0]);
                  hold on;
