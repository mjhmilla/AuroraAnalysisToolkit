...
for idxExp =1:1:length(experimentsToProcess)
  ...
  for idxTrial=1:1:length(expJson.trials)
    ...
    for idxTag=1:1:length(trialJson.experiment.tags)
      if(strcmp(trialJson.experiment.tags{idxTag},...
                'isometric-active-passive-force-length'))
        flag_containsForceLengthData=1;
      end
    end
  end
  if(flag_containsForceLengthData==1)
...
for idxExp = 1:1:length(experimentsToProcess)
  ...
  %
  % Identify the trials tagged with 
  %     active-force-length  
  %             or 
